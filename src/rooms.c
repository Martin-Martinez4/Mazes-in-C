...
int bvh_insert(BVHNodes* nodes, Rooms* rooms, int nodeIndex, int roomIndex) {
  ...
  if (nodes->data[nodeIndex].room_index != -1) {
    ...
    BVHNode newLeaf = {
        .box = rooms->data[roomIndex].aabb, .left = -1, .right = -1, .room_index = roomIndex};
    ...
    node->room_index = -1;
    node->box        = aabb_union(nodes->data[oldLeafIndex].box, nodes->data[newLeafIndex].box);
  ...
  double expandRight =
      aabb_expansion_cost(nodes->data[node->right].box, rooms->data[roomIndex].aabb);
