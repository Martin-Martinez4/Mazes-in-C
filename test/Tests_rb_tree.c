...
void Test_traverseInOrder() {
  ...
  for (int i = 0; i < lenTestCases; i++) {
    ...
    if (written != tc.length) {
      fprintf(stderr,
              "=====\n\033[31mAssertion failed: Test Case: %d, file %s, line %d. Message: length "
              "not the same written: %d, want: %d\033[0m\n=====\n",
    ...
    for (int j = 0; j < written; j++) {
      if (strcmp(got[j], tc.want[j]) != 0) {
        fprintf(stderr,
                "\t=====\n\033[31mAssertion failed: Test Case: %d, index: %d; got: %s, wanted: "
                "%s\033[0m\n=====\n",
                i, j, got[j], tc.want[j]);
        allPassed = false;
      }
  ...
  for (int i = 0; i < lenTestCases; i++) {
    ...
    for (int j = 0; j < written; j++) {
      ...
      if (got[j]->red != tc.wantIsRed[j]) {
        fprintf(stderr,
                "\t=====\n\033[31mAssertion failed: Test Case: %d, index: %d; got: %d, wanted: "
                "%d\033[0m\n=====\n",
...
void Test_searchRBTree() {
  ...
  for (int i = 0; i < lenTestCases; i++) {
    ...
    for (int j = 0; j < tc.testLength; j++) {
      ...
      if (got != tc.wants[j]) {
        fprintf(stderr,
                "\t=====\n\033[31mAssertion failed: Test Case: %d, search #%d; Looking for: %s "
                "got: %d, wanted: %d\033[0m\n=====\n",
